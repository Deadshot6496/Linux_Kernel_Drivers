...
char data[128]; // Holding the data buffer for the device file.
...
static ssize_t etx_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
        pr_info("Driver Read Function Called...!!!\ Offset -> %d\n",*off);
	int to_copy = (len + *off) < sizeof(data) ? len : (sizeof(data)-*off);
	if(*off >= sizeof(data))
	{
		return 0;
	}
	int not_copied = copy_to_user(buf,&data[*off],to_copy);
	pr_info("Buffer = %s\n",buf);
	int delta = to_copy-not_copied;
	pr_warn("Not copied count = %d", delta);
	*off += delta;
...
{
        pr_info("Driver Write Function Called...!!! Offset -> %d\n",*off);
	int to_copy = (len + *off) < sizeof(data) ? len : (sizeof(data)-*off);
	if(*off >= sizeof(data))
	{
		return 0;
	}
	int not_copied = copy_from_user(&data[*off],buf,to_copy);
	int delta = to_copy-not_copied;
	pr_warn("Not copied count = %d", delta);
	*off += delta;
        return delta;
...
dev_t dev =0;  // 32 bit integer where first 12 bits are major and remaining 20 bits are minor Number
...
static int __init hello_world_init(void)
{
	...
	dev_class = class_create("My_Char_Device");
	if(IS_ERR(dev_class)){
	 goto r_class;
	}
